...
- (void)setupOneMediaTypeSelection
{
    if(_maximumNumberOfSelectionMedia > 0)
    {
        ...
        self.labelSelectedMedia.text = @"选择相片/视频";
    }
    else
    {
        if(_maximumNumberOfSelectionPhoto ==0)
        {
            ...
            self.segmentedControl.hidden = YES;
            self.labelSelectedMedia.hidden = NO;
            self.labelSelectedMedia.text = @"选择视频";
        }
        else if(_maximumNumberOfSelectionVideo ==0)
        {
            ...
            self.segmentedControl.hidden = YES;
            self.labelSelectedMedia.hidden = NO;
            self.labelSelectedMedia.text=[NSString stringWithFormat:@"最多只能选 %ld 张相片",(long)_maximumNumberOfSelectionPhoto];
        }
        else
        {
...
- (void)setupCollectionView
{
    ...
    NSInteger iBottomViewH= self.bottomView.frame.size.height;
    NSInteger iNavigationTopH=self.navigationTop.frame.size.height;
    
    self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0,iNavigationTopH, self.view.bounds.size.width, self.view.bounds.size.height-iNavigationTopH-iBottomViewH) collectionViewLayout:layout];
...
- (void)setTitle:(NSString *)title
{
    ...
    //[self.btnTitle setImageEdgeInsets:UIEdgeInsetsMake(5, self.btnTitle.titleLabel.frame.origin.x +self.btnTitle.titleLabel.frame.size.width + self.btnTitle.imageView.frame.size.width, 0, 0)];
    //[self.btnTitle setTitleEdgeInsets:UIEdgeInsetsMake(5, 10, 0, 0)];
...
- (IBAction)btnAction:(id)sender {
    ...
    switch (btn.tag) {
        ...
        {
            //判断是否超过最多能选相片数量
            if (self.collectionView.indexPathsForSelectedItems.count==_maximumNumberOfSelectionPhoto) {
                UIAlertView *alert=[[UIAlertView alloc] initWithTitle:nil message:[NSString stringWithFormat:@"最多只能选 %ld 张相片",(long)_maximumNumberOfSelectionPhoto] delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
                [alert show];
                return;
            }
